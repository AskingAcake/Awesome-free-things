...
| 4 | [WMO WWIS](https://worldweather.wmo.int/) | Unlimited (no published cap) | NO | Yes | No | Global | Yes | Direct UN/govt source; raw JSON by city ID |
...
| 8 | [OpenWeatherMap](https://openweathermap.org/price) | 1M/month, 60/min (Free Weather API path); 1K/day (One Call 3.0/4.0 path) | YES | 3-hour intervals | Yes | Global | Limited | Two separate product paths with different limits — do not mix |
...
| 12 | [Tomorrow.io](https://www.tomorrow.io/weather-api/) | 500/day (no weekly/monthly cap beyond daily) | YES | 14 days | Core params only on free | Global | Non-commercial | Pollen, lightning, air quality are paid only |
...
| 16 | [ECMWF Open Data](https://www.ecmwf.int/en/forecasts/datasets/open-data) | Free subset (no published cap) | YES (free) | HRES + ENS | Limited | Global | CC BY 4.0 | Gold-standard European Model open subset |
...
| 19 | [MET Norway / Yr.no](https://api.met.no/) | Unlimited (caching encouraged; no hard cap published) | NO | Yes | No | Global (best Arctic) | CC license | Backend for many pro apps; very accurate Arctic/Nordic data |
...
| 21 | [Synoptic Open Access](https://synopticdata.com/open-access) | Free (academic only; no published hard cap) | YES (free) | No | Yes | Global | Academic only | 100K+ stations; QC-filtered data; students/researchers |
...
| 26 | [AVWX.rest](https://avwx.rest/) | Free hobby tier; daily cap (reset 00:00Z); exact number not published | YES (free) | TAF (aviation) | No | Global | Limited | Basic METAR + TAF parsing free; other features require paid plan |
...
| 36 | [Xweather / AerisWeather](https://www.xweather.com/) | Free tier (exact limit not publicly published; must sign up to see) | YES | Yes | Yes | Global | Limited | Best free mapping + weather visualization layers |
| 37 | [Pirate Weather](https://pirateweather.net/) | 20K/month (~666/day) | YES | Yes | 7 days back | Global | Limited | Drop-in Dark Sky replacement; same JSON format; open source |
...
| 42 | [Farmonaut](https://farmonaut.com/api-development/) | Free tier (exact limit not publicly published; must sign up to see) | YES | Yes | 20 years | Global | Limited | Agri + NDVI + EVI + GDD + soil moisture + weather combo |
...
| 46 | [UK Met Office DataHub](https://datahub.metoffice.gov.uk/) | Free tier (exact daily limit not publicly published; must sign up to see) | YES (free) | Yes | Yes (UK obs) | Global | Limited | Replaced old DataPoint API; 45K global forecast locations |
| 47 | [SILO / Australian Long Paddock](https://www.longpaddock.qld.gov.au/silo/) | Unlimited (email address required in query instead of key) | NO (email in query) | No | From 1889 | Australia | CC BY | Queensland govt; 0.05 degree resolution; longest free archive |
| 48 | [DPIRD](https://www.dpird.wa.gov.au/online-tools/apis/) | Free tier (exact limit not publicly published; must sign up to see) | YES (free) | Yes | Yes | Western Australia | Limited | WA agricultural weather station network; high-quality regional data |
| 49 | [weather.tsukumijima.net](https://weather.tsukumijima.net/) | Unlimited (0.5s minimum between requests) | NO | Yes | No | Japan | Unclear | Unofficial JMA scraper; clean JSON; almost zero English docs |
| 50 | [weather-au](https://github.com/tonyallan/weather-au) | Unlimited (no published cap; unofficial) | NO | Yes | Yes | Australia | Non-commercial | Unofficial Python lib hitting BOM unpublished beta API endpoints |
...
| 58 | [Meteomatics](https://www.meteomatics.com/en/weather-api/) | TRIAL ONLY — 14 days, 500 queries/day, 50/min, 10 parallel | YES | Yes | Yes | Global | Trial only | 1800+ parameters; widest parameter list of any API — but not a true free tier |
